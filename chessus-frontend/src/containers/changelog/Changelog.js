...
const changelogData = [
  {
    ...
    items: [
      ...
      "Solving a puzzle for the first time now moves your puzzle rating, which is kept separate from your game rating. Only your first attempt at a given puzzle counts, so it can't be farmed. Gains get smaller as your rating climbs and losses get smaller as it falls, so the number settles around how often you actually solve things.",
      "Puzzles pick up a rating of their own — the average rating of the people who have solved them. It stays hidden until at least 10 people have solved it, since it means nothing before that, and creators can keep it hidden after that too.",
      "Finished games now have a 'Create puzzle from this position' button that opens the builder with that exact position loaded, including the move that led into it. Step back through the replay first to pick the moment you want.",
      "Puzzles can now run up to 8 moves a side, not just one. In the builder, \u201cSet the solution\u201d plays the line forward: record your move, then the reply you expect, then your next move, and so on \u2014 the board carries on from each one so you are always choosing from the position in front of you. The line is listed as you build it, with undo.",
      "Solving a longer puzzle goes one move at a time. Play the move you think works and, if it is right, your opponent answers on the board and you carry on. The rest of the answer is never sent to your browser before you have found it \u2014 only the reply to a move you have already played.",
      "Because the pieces here are made by users, there is no engine to work out the opponent's best defence, so the replies in a longer puzzle are the ones its creator wrote. Checking a long line confirms that every move in it can actually be played, and says which one cannot when something is off, but whether the opponent could have defended better is the creator's call.",
      "Partial credit now applies where it means something: stopping half way through a two-move puzzle costs less rating than not finding the first move at all, and finishing it later turns that partial credit into the full gain. Walking away part-way through still counts \u2014 your first attempt is your first attempt.",
      "The puzzle board now behaves like a game board: hovering a piece shows where it can go, and you can drag a piece to its square instead of clicking twice. When you find the answer the move is played out on the board rather than the piece snapping back.",
      "Longer puzzles now give partial credit. Your rating moves by how much of the line you found, counted from the first move — miss the opening move and it still counts as a miss, since that is the idea the puzzle is testing.",
      "Fixed pieces appearing to have no moves at all inside a puzzle: the puzzle board was reading the piece definitions under the wrong field names, so a knight, for example, generated nothing. This affected the checkmate checker as well, which could report a genuine mate-in-1 as unsolvable.",
      "Fixed pieces on a puzzle board showing as 'undefined' when you hovered them.",
  ...
  },
  {
    ...
    items: [
      ...
      "On tall, narrow boards the board panel no longer stretches across the page. It now sizes to the board — never narrower than the Game Settings card below it, and lined up squarely above it — with the board and its clocks and buttons centred inside, and the clocks and buttons centred vertically alongside the board. Zooming in widens the panel to match, so a zoomed board is no longer cut off at its unzoomed width.",
    ],
  },
  {
    ...
    items: [
      ...
      "Fixed a way pieces could silently lose a direction: if a direction was set to infinite (∞) and you then ticked \"exact\" for it, that direction was reset to 0 — switching it off entirely rather than switching it from infinite to an exact distance. It now falls back to 1 so the direction stays enabled. This applied to movement, capture, and attack range.",
  ...
  },
  {
    ...
    items: [
      ...
      "Move dots now scale with the board, so they no longer look oversized on a small phone board or tiny on a large zoomed-in one. They're also a little more transparent, and the first-move-only dot now uses the site's muted gold instead of a bright yellow.",
      "Fixed: dots didn't appear on squares holding an enemy piece that can't be captured directly — most visibly, the piece delivering checkmate looked like it was attacking nothing. Dots now show wherever a piece can move or attack in normal circumstances, enemy pieces included.",
      ...
      "New replay option, 'Hide illegal moves'. By default the dots describe the hovered piece itself — everywhere its own movement and attack patterns reach. Turn it on to apply the board's rules as well: targets that can't be captured, pieces that must be checkmated rather than taken, impassable and restriction-zone squares, and moves that would leave that side's game-losing piece capturable.",
      ...
      "Fixed: the piece uniqueness checker and 'Compare Pieces' could disagree — a piece could be reported as unique while the comparer showed it had no functional differences from an existing piece at all. Both now use exactly the same comparison, so their answers always match.",
  ...
  },
  {
    ...
    items: [
      ...
      "Reactive Veto Power vs. the computer is now more natural: the computer only vetoes the move you actually played (if it's one worth denying), instead of marking moves you didn't make — this now applies to all reactive games, including ones with a per-game veto limit.",
      ...
      "Veto Power now works fully against the computer, including Fairy Stockfish, in both pre-emptive and reactive styles. Fixed pre-emptive games getting stuck on 'Opponent is choosing vetoes' — the computer now commits its vetoes when it's your turn so you can see them and move, its own move is held so you can veto it, and it vetoes your moves too.",
      ...
      "The veto mechanic is now called 'Veto Ability' (previously 'Veto Power') across the game setup and info pages.",
      "The 'Veto this move' button now lives on its own row in the Actions panel and is greyed out when it isn't your turn to veto, matching the other action buttons.",
      "Veto vs. the computer now blocks its move for real: when you veto a Fairy Stockfish move, it no longer 'thinks again' and plays the same vetoed move — it now picks a genuinely different, ban-respecting move. In pre-emptive games the computer waits for your veto first and then plays around it.",
      "Fixed a veto display bug where the computer's veto could briefly flash on the board — sometimes even on one of your own moves. Veto marks now only show on the board of the player whose moves were actually banned.",
      "The move-direction arrow indicator is now off by default for new accounts. You can turn it on any time under Preferences.",
    ],
  },
  {
    date: "August 31, 2026",
  ...
  },
  {
    ...
    items: [
      "Game wizard: new 'Surround Capture' mechanic — a piece (or connected group) is captured and removed when it has no adjacent empty space left. This is the capture method used by Go. Optional diagonal adjacency and a 'no self-capture' placement rule (Go's suicide rule).",
      "Game wizard: new 'Forbid Repeating Positions' mechanic — a move that recreates an earlier board position is rejected (Go's ko rule, or full positional superko).",
      "Game wizard: new 'Allow Pass' mechanic — players can pass their turn, and the game can end after a set number of consecutive passes (how a game of Go ends). A Pass button appears in-game when enabled.",
      "Game wizard: new 'Enclosed Region Scoring' — empty areas fully surrounded by one player count toward their score at game end, with Area (Chinese) or Region (Japanese) scoring models.",
      "Game wizard: new 'Highest Score Wins' win condition — when the game ends (e.g. by consecutive passes) the higher score wins; equal scores draw.",
      "Points are now fractional: points-to-win, per-player starting points (handicap / komi), capture points, and control-square points all accept decimals like 6.5.",
      "Bots (easy/medium/hard) understand all of the above — they capture by surrounding, avoid illegal self-capture and repeated positions, pass when appropriate, and play for territory in region-scored games.",
      "Game detail pages now describe all of these new mechanics when a game uses them.",
      "Game wizard (Step 3): new display option to render the board Go-style — a wood-coloured grid of lines with pieces shown on the intersections instead of inside coloured squares.",
      "Bots can now actually play territory / enclosure placement games (Go-style): fixed a bug that could leave the computer unable to find any move, and added a dedicated placement strategy (easy plays loosely, medium/hard favour captures, ataris, connection and territory) so the computer always makes a sensible move or passes.",
      "Game wizard: 'Enclosed Region Scoring' now lives as a sub-option under the 'Highest Score Wins' win condition, and the scoring-model / repetition-scope choices use clearer radio buttons.",
      "Game wizard: fixed placeable-piece duplicates in non-reserve games — you can add a piece once per specific player, but adding it for 'All Players' (or when it's already available to everyone) is now blocked with a clear message.",
      "Game library cards now list score-based (Highest Score) win conditions, including the scoring model.",
      "Live games with 'Highest Score Wins' now show a score widget under the board with each player's running score and the scoring model, so it's clear what's being counted.",
      "Passing is now shown in the move list and in the game/match history on player profiles.",
      "Fixed 'Highest Score Wins' games declaring the wrong winner — territory and stones weren't being counted in the final score. The final score now decides the winner correctly.",
      "The final score of both players is now shown in the game-over screen, on match-history cards, and on the match detail page.",
      "Win/loss reasons (including highest-score and pass endings) now display on match-history cards and the match detail page.",
      "Fixed the in-game Pass button being cut off the side of the sidebar — it now sits on its own row in the Actions panel.",
      "Fixed unwanted horizontal page scrolling caused by the move-history sidebar.",
  ...
  },
  {
    ...
    items: [
      "Hidden Enemy Pieces mode: you can now probe with attack-only moves (e.g. a pawn diagonal into a square that looks empty, or any piece with a capture pattern that differs from its movement). If a hidden enemy is actually there, the capture succeeds; if the square is truly empty, the attempt counts toward your illegal-move limit — same way premoves currently behave.",
      "Computer opponents are noticeably sharper at noticing free pieces. The easy/medium/hard bots now run an 'instant tactical' check before each move, so any piece that can be lost (or captured) for at least bishop value is rescued or grabbed immediately instead of being missed during the regular search. Medium's search depth and easy's quiescence depth were also bumped one ply, and easy's random-move chance was reduced from 35% to 20%. The hanging-piece penalty in the static evaluation was raised so positional gains can't quietly outweigh losing material.",
      "Game library cards now show the 'Illegal Move Limit' win condition (and its limit) when a game type has it enabled.",
      "Default castling distance now scales with board width: width ≤ 6 → 1 square, width 7–8 → 2, width 9–11 → 3, and +1 for every two additional columns (12–13 → 4, 14–15 → 5, etc.).",
      "New game-design options inspired by Tsuitate Shogi (screen shogi): 'Hidden Enemy Pieces' completely conceals opponent pieces, moves, captures, and check sources during the live game (everything is revealed once the game ends), and a new 'Illegal Move Limit' win condition (1–100, default 8) that costs you the game when you attempt that many illegal moves. The two pair naturally — when you can't see what's blocking you, every probe is a gamble.",
      "Host-game modal: new 'Spectator visibility' option for games with Hidden Enemy Pieces or Fog of War, letting the host choose whether spectators see the full board or are locked to Player 1 / Player 2's view.",
      "Hidden Enemy Pieces mode: premoves are disabled (you can't queue a move at a piece you can't see), the captured-pieces panel is hidden until the game ends, opponent move history is anonymized to '… x' / '… +' style markers, and illegal-move attempts surface as a private toast counter rather than an error.",
      "Critical fix: when Fairy Stockfish submitted a move the server couldn't legalize (Betza model didn't capture the full rule), the server silently rejected it and the client's engine would resubmit the same illegal move every second, stalling the bot turn while the human's clock continued to tick. The server now (a) tells the client when its engine move was rejected, (b) refuses to spin up duplicate bot-turn handlers for the same game, and (c) defers to the existing server-side fallback bot for that one turn so the game keeps moving.",
      ...
      "Premoves: fixed a case where a premove that became illegal (after the opponent's move) silently failed — the yellow highlight could stick, no notice was shown, and the player had to re-make the move manually. Premove cancellations are now broadcast game-wide and always clear the highlight; a brief 'Premove cancelled' toast appears with the reason.",
      "Match history and the match-detail page now display 'Illegal Move Limit' as the game's win reason when that condition triggered the result.",
      "Castling vs. regular move on the same square: when a piece can both move and castle to the same destination, that square now shows a small extra dot above the normal move dot. A quick click still performs the regular move; press and hold for 1 second to castle instead — a yellow ring fills around the cursor during the hold and the dot brightens once castling is armed. Release early to reset and try again.",
      "Fixed close-range castling: when a piece could both move 1 square AND castle 1 square (e.g. king with castling distance 1 next to its castling partner), the server was rejecting the castle because the partner currently occupied the king's target square. The castle path now correctly ignores the partner's square as a blocker.",
      "Hidden Enemy Pieces: premoves are now allowed (previously they were disabled in HEP games). If your premove turns out to be illegal once the opponent's move is revealed, the premove is silently cancelled — and if the game has the Illegal Move Limit win condition enabled, the failed premove counts toward your limit just like any other illegal probe.",
      "Sounds: fixed an issue where some moves played no sound. Move and capture sounds were being clipped to 250–600 ms whether or not the browser had actually started playback yet, which caused the clip to be cut off in the middle (or skipped entirely) when the audio system was under load. The concurrent-sound cap was also raised so bursts of moves no longer drop sounds, and pending sounds queued while the tab was in the background are now all replayed on return instead of just the last one.",
      "Computer opponents (all difficulty levels) no longer favor edge-file pawn pushes (a-pawn / h-pawn-style) over central pawn pushes. The position evaluator now explicitly penalizes low-value pieces that have left the back rank onto an edge file, since they can contribute almost nothing to central control.",
      "Castling rules simplified: the path between the king and its castling partner no longer needs to be clear — only the king's actual travel path matters. This makes short-range castling distances (e.g. distance 1 with the partner sitting at the corner) work correctly, since the partner now always teleports to the king's other side regardless of where it started.",
      "Castling UI: clicking directly on the castling partner now executes the castle immediately (useful when castling distance equals 1, where the partner sits on the king's destination). The regular move dot now disappears entirely once the 1-second hold arms the castle, so it's clear which move will fire.",
      "Move history in Hidden Enemy Pieces games now shows a bare '…' for every opponent move — no capture marker, no castling marker, no check indicator. The previous '… x' / '… +' style markers were leaking information about what kind of move was made.",
      "Hidden Enemy Pieces: the move-history step-through (analysis) feature now correctly hides opponent pieces while the game is still in progress. Previously the initial board snapshot used for replay was sent unfiltered, so stepping back through history would reveal the opponent's starting positions. Full board reveal still happens once the game ends.",
    ],
  },
  {
    date: "May 28, 2026",
  ...
  },
  {
    ...
    items: [
      ...
      "Fixed: pieces with custom movement squares were allowed to capture on those squares if they had 'capture on move' enabled. Custom movement squares are movement-only by design — captures must come from custom attack squares. This affected live games, the AI, and the piece-value estimator.",
      "Fairy Stockfish now supports pieces with custom movement and attack squares. Symmetric and half/quadrant-symmetric leaper patterns (e.g. a knight-like piece restricted to forward moves only) are translated into Betza notation and accepted by the engine. Patterns that can't be expressed as a leaper subset still mark the game as incompatible with a clear explanation.",
      "Fixed: Fairy Stockfish failed to start in production with a 'SharedArrayBuffer is not defined' error, even on game types the lobby showed as compatible. The production server was missing the cross-origin isolation headers required by the threaded WebAssembly build; those headers are now sent by Express in both dev and prod.",
    ],
  },
  {
    date: "May 19, 2026",
