...
const NewPuzzle = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        /*
         * The games this person made. Puzzles can technically be built on any
         * game, but the allowance is counted per game and the site describes it
         * as building on your own - so offering someone else's here would
         * promise something the copy does not.
         */
        const { data } = await axios.get(
          `${API_URL}games?creatorId=${currentUser.id}&limit=200`,
          { headers: authHeader() }
        );
        if (cancelled) return;
        const list = Array.isArray(data) ? data : (data?.games || data?.rows || []);
        setGames(list.filter(Boolean));
      } catch (_) {
  ...
  return (
    <div className={styles["page"]}>
      ...
      <p className={styles["sub"]}>
        Choose the game your puzzle is played on. Its pieces, board and rules are what
        the puzzle will use, so the solver plays it exactly as the game is.
      </p>

      {loading && <p className={styles["empty"]}>Loading your games…</p>}

      {!loading && failed && (
        <p className={styles["empty"]}>Could not load your games just now. Try again in a moment.</p>
      )}
      ...
      {!loading && !failed && games.length === 0 && (
        <div className={styles["none"]}>
          <p className={styles["none-title"]}>You have no games to build a puzzle for.</p>
      ...
      {!loading && !failed && games.length > 0 && (
        <div className={styles["picker"]} ref={boxRef}>
          ...
          <input
            ...
            autoComplete="off"
            placeholder={games.length > 6 ? 'Search your games…' : 'Pick a game…'}
            value={query}
          ...
          {open && (
            <ul className={styles["list"]} id="puzzle-game-list" role="listbox">
              {matches.length === 0 && (
                <li className={styles["no-match"]}>No game of yours matches “{query.trim()}”.</li>
              )}
              {matches.map((g, i) => (
                <li key={g.id}>
                  <button
                    ...
                    onClick={() => choose(g)}
                  >
                    <span className={styles["option-name"]}>{nameOf(g)}</span>
