...
function checkCompatibility(gameType, pieceDefs = [], placements = []) {
  ...
  // Not safe to ignore: the engine would move any piece, where the rule says
  // it must move the type its opponent chose. See server/designated-piece.js.
  if (parseJsonSafe(gameType.other_game_data, {})?.designate_piece_type === true) pushGame('designate_piece_type',
    'Opponent chooses the piece type: the engine cannot be told which piece it must move.',
    'Turn off "Opponent chooses the piece type" in the game wizard (Step 2), or play the built-in bot.');
